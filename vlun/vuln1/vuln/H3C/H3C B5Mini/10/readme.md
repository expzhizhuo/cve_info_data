...
The H3C B5 Mini B5MiniV100R005 router was found to have a stack overflow vulnerability in the Edit_BasicSSID_5G function. An attacker can obtain a stable root shell through a carefully constructed payload.

![image-20220720211305469](img/image-20220720211305469.png)

In the `Edit_BasicSSID_5G` function, `V30`(the value`param`) we entered is formatted using the `sscanf` function and in the form of `%[^;]`. This greedy matching mechanism is not secure, as long as the size of the data we enter is larger than the size of `V31`, it will cause a stack overflow.
