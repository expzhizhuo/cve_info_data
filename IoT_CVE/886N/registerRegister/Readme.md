...
This vulnerability is on the /cloud_config/router_post/register interface and affects the latest version of the router operating system (which is a different RTOS system from the linux operating system), and the firmware version is TL-WR886N V6.0 upgrade software 20190826 2.3.8
## Vulnerability description
There is a buffer overflow vulnerability on the /cloud_config/router_post/register interface.

1. An interface ```/cloud_config/router_post/register``` is registered first in the registerRegister function.
![](./registerRegister1.png)
